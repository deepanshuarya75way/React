...
const Auth = () => {
    ...
    useEffect(() => {
        if (isLoginSuccess) {
            ...
            dispatch(setUser({token: loginData?.token, name: loginData?.name, email: loginData?.email}))
            navigate('/dashboard')
        }
        if (isLoginError) {
    ...
    const { register, handleSubmit, formState: { errors } } = useForm<LoginFormFields>({ resolver: yupResolver(schema) })
